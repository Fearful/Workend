...
export const actions = {
  createRole: async ({ request, params, cookies }: import('./$types').RequestEvent) => {
  ...
  updateRole: async ({ request, cookies }: import('./$types').RequestEvent) => {
    const fd = await request.formData();
  ...
  deleteRole: async ({ request, cookies }: import('./$types').RequestEvent) => {
    const fd = await request.formData();
    const roleId = String(fd.get('role_id') || '');
    if (!roleId) return fail(400, { roleError: 'Role ID required' });
  ...
  assignRole: async ({ request, params, cookies }: import('./$types').RequestEvent) => {
    const fd = await request.formData();
  ...
  generateKey: async ({ params, cookies }: import('./$types').RequestEvent) => {
    const cookie = cookies.get(SESSION_COOKIE);
    const cookieHeader = cookie ? `${SESSION_COOKIE}=${cookie}` : undefined;
  ...
  revokeKey: async ({ request, cookies }: import('./$types').RequestEvent) => {
    const fd = await request.formData();
